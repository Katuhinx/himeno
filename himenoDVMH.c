...
# pragma dvm array align ([] [ i ][ j ] [ k ] with bnd [ i ][ j ][ k ]) 
...
# pragma dvm array align ([ i ][ j ] [ k ] with bnd [ i ][ j ][ k ]) ,shadow [1][1][1]
...
main()
{
  ...
  nn= 1;//100 итерации алгоритма Якоби
...
initmt()
{
    ...
    #pragma dvm region
    {
   #pragma dvm parallel ([ i ][ j ] [ k ] on p [ i ][ j ][ k ])
  for(i=0 ; i<MIMAX ; i++)
    for(j=0 ; j<MJMAX ; j++)
      for(k=0 ; k<MKMAX ; k++){
        a[0][i][j][k]=0.0;
...
jacobi(int nn)
{
  ...
  for(n=0 ; n<nn ; ++n){
    gosa = 0.0;
#pragma dvm actual ( gosa )
#pragma dvm region
{
 #pragma dvm parallel ([ i ][ j ] [ k ] on p [ i ][ j ][ k ]) shadow_renew(p(corner)) reduction ( sum ( gosa ) ) private(s0, ss)
    for(i=1 ; i<imax-1 ; i++)
      for(j=1 ; j<jmax-1 ; j++)
        for(k=1 ; k<kmax-1 ; k++){
